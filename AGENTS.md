...
- Изменить дефолты чекбоксов/полей — пользователи зависят от текущих значений. Дефолт
  `cfgTun: checked` — осознанный и подтверждён владельцем 2026-09-15: hint выправлен на
  «TUN включён по умолчанию; MIPS и опции «на каждый прокси» выключены» (ранее hint
  ошибочно утверждал обратное). Дефолты больше не менять мимоходом.
