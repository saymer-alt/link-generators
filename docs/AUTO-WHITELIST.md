...
без подписок отсутствует `use`. URL следует выбранному Ping server.
Сам `GLOBAL` намеренно **не задаёт `expected-status`**: для приоритетного failover
достаточно успешно установить маршрут через proxy и получить HTTP-ответ; точный код
ответа не должен превращать рабочий маршрут в `dead`. Это также приближает плановую
проверку к ручному Ping MetaCubeXD (`/delay`), который не передаёт expected-status.
Строгая проверка `expected-status` у HTTP providers сохраняется отдельно.
...
- Совместимость WG/AWG: полевой кейс 23.09.2026 показал, что ручной Ping мог
  временно оживлять AWG, после чего следующая строгая автоматическая проверка снова
  помечала его недоступным. На `main` проверяется более мягкая group-level семантика
  без `expected-status`; до полевого подтверждения это кандидат на исправление, а не
  доказанная причина конкретного туннеля.
...
- Production-like лаборатория на Mihomo 1.19.31 с generated-параметрами
  `interval: 300`, `lazy: false`, `timeout: 60000`, `max-failed-times: 2`
  показала асимметричную задержку обнаружения TCP blackhole: при полном отсутствии
  пользовательского трафика PRIMARY → FALLBACK занял **304896 мс** (~5м04.9с),
  то есть примерно один health-интервал плюс provider check; при повторяющихся
  пользовательских подключениях тот же blackhole переключился примерно за **15 с**
  после двух ~5-секундных dial failures и внеплановой проверки. Это измерения
  конкретной лаборатории, а не гарантированный SLA.
- Перезапуск Mihomo не является требованием для failover: он запускает свежую
  health-проверку и поэтому может визуально «починить» пассивный blackhole за секунды.
  Ручной выбор — отдельное состояние: вручную выбранный FALLBACK может пережить
  restart через `store-selected`; fixed-выбор снимается, когда выбранная цель
  становится dead для test URL.
