...
| 🌐 Modern REALITY | `realityModernInput` | `mihomoRealityModernHosts` | пусто | multiline `host` / `host:port` / `[ipv6]:port`: только REALITY-узлы этих серверов получают `support-x25519mlkem768: true` + chrome fp (если не задан) и override-expr у провайдеров; пусто — legacy; сервер Xray v25.5.16+ (NEEDS_OWNER: окно совместимости версий уточняется владельцем — floor v25.5.16 подтверждён release notes Xray, ceiling не подтверждён) |
