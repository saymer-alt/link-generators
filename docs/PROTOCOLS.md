...
Полная [матрица 28 outbound types, AWG-аудит и proposal](AUDIT-MIHOMO-1.19.31.md)
сверены с тегом Mihomo v1.19.31 (2026-09-14). Таблица ниже описывает именно
поддерживаемые входы генератора, а не весь набор протоколов ядра.
SSR/Hysteria v1/OpenVPN/SSH и специализированные сетевые профили пока не добавлены.
AWG 3.1 проверен по всему UI pipeline: семь строковых полей (включая диапазоны),
два boolean, официальный `on/off`, автоматический `version: 3`. Новых изменений AWG runtime нет.
Прямой `parseWireGuardConf` без нормализаторов страницы не эквивалентен этому pipeline.
...
Поддержка AWG 3.1 начинается с Mihomo 1.19.30; текущий целевой и тестовый
baseline проекта — Mihomo v1.19.31, где 3.1 поддерживается. Не смешивать три уровня версии: пакетная версия opkg (в entware-go
