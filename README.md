...
2. Вставьте proxy-ссылки. Режим **📡 URL-подписки (Sub Mode)** включён по умолчанию: оставьте его включённым, если используете HTTP(S) URL подписки. Если в поле только обычные `vless://`, `trojan://`, `ss://` и другие proxy-ссылки — выключите режим.
...
- **HTTP(S)-подписки** — в режиме **📡 URL-подписки (Sub Mode)**; генератор прописывает их как `proxy-providers`, скачивать подписку будет сам Mihomo, а не страница.
...
proxy-ссылок отключите **📡 URL-подписки (Sub Mode)**.
...
в режиме **📡 URL-подписки (Sub Mode)** превращаются в `proxy-providers`. Подписку скачивает сам Mihomo;
...
- 📡 **URL-подписки (Sub Mode)** включены по умолчанию. В обычном режиме при вводе только proxy-ссылок их следует выключить; для HTTP(S) URL подписок — оставить включёнными.
