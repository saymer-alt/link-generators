...
- **⚡ MIPS stack для TUN** — включён по умолчанию для **Mihomo >= 1.19.31** (на более старой версии выберите gVisor / отключите MIPS — предупреждение показывается, генерацию не блокирует); **gVisor** остаётся compatibility fallback. MIPS здесь — **TUN stack, не архитектура CPU**. Доступен только при включённом TUN. Работает для обычного TUN и «TUN на каждый прокси», включая профиль VPS Gateway. Значения `system`/`mixed` спрятаны за крышкой «⚙ Расширенный TUN stack».
