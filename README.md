...
- **📡 Sub Mode** — HTTP(S)-подписки как `proxy-providers`: обновление раз в 12 часов, встроенный health-check. Включён по умолчанию; **для обычных ссылок отключите**. Рядом — **🚫 Exclude Filter**: regexp/keyword-фильтр имён узлов в каждой подписке (пусто — не используется).
...
- В БС-режиме опции «TUN на каждый прокси» / «SOCKS-порт на каждый прокси» и VPS Gateway недоступны; обычный TUN, gVisor/MIPS, Mixed Port, Web UI, Allow LAN и Sub Mode сохраняются.
