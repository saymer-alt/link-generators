...
- **⚡ MIPS stack для TUN** — opt-in для **Mihomo >= 1.19.31**, выключен по умолчанию; стандартом остаётся **gVisor**. MIPS здесь — **TUN stack, не архитектура CPU**. Доступен только при включённом TUN. Работает для обычного TUN и «TUN на каждый прокси», включая профиль VPS Gateway.
