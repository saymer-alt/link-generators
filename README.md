...
- **📡 Использовать URL-подписки** — HTTP(S)-подписки как `proxy-providers`: обновление раз в 12 часов, встроенный health-check. Включён по умолчанию. В обычном режиме, если используются только proxy-ссылки без URL подписок, его следует выключить. Смешанный ввод поддерживается там, где есть хотя бы один URL подписки. Рядом — **🚫 Exclude Filter**: regexp/keyword-фильтр имён узлов в каждой подписке (пусто — не используется), а **📱 Device Model** — опциональное значение для HTTP-заголовка `x-device-model`, передаваемого каждому subscription provider; пустое поле сохраняет прежний вывод.
