...
- **🛡️ TUN** и **🔒 TUN на каждый прокси** (группа «Отдельный вход на каждый прокси») — системный туннель либо отдельный TUN-интерфейс на каждый прокси. Зависимая опция включаема только при включённом TUN; выключение родителя сбрасывает её.
